...
import type { ActionDefinition, HttpAdapterConfig } from "./types.js";
...
export class HttpApiAdapter implements AppAdapter {
	...
	async executeAction(action: ChosenAction, ctx: ActionContext): Promise<ActionResult> {
		...
		try {
			...
			const responseData = resp.headers.get("content-type")?.includes("json") ? await resp.json() : await resp.text();
	...
	private interpolate(template: string, ctx: { member: MemberConfig; family: FamilyConfig }): string {
		return template
			.replace(/\{\{member\.meta\.(\w+)\}\}/g, (_, key) => {
				const meta = ctx.member.meta as Record<string, unknown> | undefined;
				return String(meta?.[key] ?? "");
			})
			.replace(/\{\{member\.(\w+)\}\}/g, (_, key) =>
				String((ctx.member as unknown as Record<string, unknown>)[key] ?? ""),
			)
			.replace(/\{\{family\.meta\.(\w+)\}\}/g, (_, key) => {
				const meta = ctx.family.meta as Record<string, unknown> | undefined;
				return String(meta?.[key] ?? "");
			})
			.replace(/\{\{family\.(\w+)\}\}/g, (_, key) =>
				String((ctx.family as unknown as Record<string, unknown>)[key] ?? ""),
			)
