...
interface GeneratedAction {
  ...
  method: string;
  path: string;
  params: { name: string; type: string; required: boolean; in: string; description: string; enumValues?: string[]; example?: string }[];
  weight: number;
}
...
export class SetupGenerator {
  ...
  async generateFromUrl(baseUrl: string, outputDir: string): Promise<InitResult> {
    ...
    for (const specUrl of [`${url}/openapi.json`, `${url}/swagger.json`, `${url}/api/openapi.json`, `${url}/api-docs`]) {
      try {
  ...
  private async probeEndpoints(baseUrl: string): Promise<GeneratedAction[]> {
    ...
    const results = await Promise.allSettled(
      probes.map(async (p) => {
        const resp = await fetch(`${baseUrl}${p.path}`, { signal: AbortSignal.timeout(2000), headers: { Accept: 'application/json' } });
  ...
  private extractParams(op: OpenAPIOperation, method: string): GeneratedAction['params'] {
    ...
    for (const p of op.parameters ?? []) {
      ...
      params.push({
        name: p.name, type: this.mapType(p.schema), required: p.required ?? false,
        in: p.in, description: p.description ?? p.name, enumValues: p.schema?.enum,
      });
    }
    ...
    if (method !== 'get' && op.requestBody?.content) {
      ...
      if (schema?.properties) {
        ...
        for (const [name, prop] of Object.entries(schema.properties)) {
          params.push({
            name, type: this.mapType(prop), required: req.has(name),
            in: 'body', description: name, enumValues: prop.enum,
          });
        }
      }
    }
  ...
  private buildSampleFamily(appName: string, actions: GeneratedAction[]): string {
    ...
    const features = cats.slice(0, 6).map((c) => `      - ${c}`).join('\n');
    const firstAction = actions[0]?.name ?? 'random';
    const topActions = actions.slice(0, 3).map((a) => a.name).join(', ');
