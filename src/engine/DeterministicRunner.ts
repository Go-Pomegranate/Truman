...
export class DeterministicRunner {
  ...
  async run(family: FamilyConfig, scenario: ScenarioConfig): Promise<ScenarioResult> {
    ...
    await this.emit({ type: 'scenario:start', scenarioId: scenario.id, familyId: family.id, actor: member.id, goal: scenario.goal });
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      const log: ActionLog = {
        ...
        action: step.action,
        decision: { action: step.action, reasoning: `[deterministic] step ${i + 1}/${steps.length}`, params: resolvedParams, frustration: 0, wantsToContinue: true },
        result,
    ...
    await this.emit({ type: 'session:end', familyId: family.id, memberId: member.id, sessionId, actions: steps.length });
  ...
  private evaluate(
    ...
    if (a.status !== undefined) {
      ...
      out.push({ criterion: { type: 'action_performed', action: act }, passed: ok,
        detail: ok ? `${act}: HTTP ${result.statusCode} === ${a.status}` : `${act}: expected HTTP ${a.status}, got ${result.statusCode}` });
    }

    if (a.success !== undefined) {
      const ok = result.success === a.success;
      out.push({ criterion: { type: 'action_performed', action: act }, passed: ok,
        detail: ok ? `${act}: success=${result.success}` : `${act}: expected success=${a.success}, got ${result.success}` });
    }

    if (a.bodyContains) {
      const body = JSON.stringify(result.response ?? '').toLowerCase();
      const ok = body.includes(a.bodyContains.toLowerCase());
      out.push({ criterion: { type: 'response_matches', action: act, pattern: a.bodyContains }, passed: ok,
    ...
    if (a.bodyNotContains) {
      ...
      out.push({ criterion: { type: 'response_matches', action: act, pattern: `!${a.bodyNotContains}` }, passed: ok,
        detail: ok ? `${act}: does not contain "${a.bodyNotContains}"` : `${act}: unexpectedly contains "${a.bodyNotContains}"` });
    }

    if (a.maxDuration !== undefined) {
      const ok = result.duration <= a.maxDuration;
      out.push({ criterion: { type: 'action_performed', action: act }, passed: ok,
        detail: ok ? `${act}: ${result.duration}ms <= ${a.maxDuration}ms` : `${act}: ${result.duration}ms > ${a.maxDuration}ms (too slow)` });
    }

    if (out.length === 0) {
      out.push({ criterion: { type: 'action_performed', action: act }, passed: result.success,
