import type { ScenarioConfig, ScenarioResult, SuccessCriterion } from "../types.js";
...
export class ScenarioEvaluator {
	evaluate(scenario: ScenarioConfig, history: SessionHistoryEntry[], startTime: number): ScenarioResult {
	...
	private evalResponseMatches(
		...
		const matching = history.filter((h) => h.action === action && h.success && regex.test(h.responseSnippet));
