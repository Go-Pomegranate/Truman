...
export class ActionLogger {
	...
	private buildDiagnosis(report: SimulationReport): string {
		...
		lines.push(
			`Duration: ${report.duration} | Actions: ${report.summary.totalActions} | Success: ${(report.summary.overallSuccessRate * 100).toFixed(0)}%`,
		);
		lines.push("");
		...
		for (const log of this.logs) {
			...
			npc.sessions.get(log.sessionId)?.push({
				...
				result: {
					...
					responsePreview:
						typeof log.result.response === "string"
	...
	private buildSummary(families: FamilyReport[]): ReportSummary {
		...
		const totalSuccesses = allMembers.reduce((sum, m) => sum + Math.round(m.actionsPerformed * m.successRate), 0);
