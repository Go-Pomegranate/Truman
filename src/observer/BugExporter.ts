...
export class BugExporter {
  ...
  export(report: SimulationReport): BugReport[] {
    ...
    for (const [action, failures] of failuresByAction) {
      ...
      if (uniqueMembers.length >= 3) severity = 1; // critical: affects all users
      else if (uniqueMembers.length >= 2) severity = 2; // high: affects multiple users
  ...
  toMarkdown(report: SimulationReport): string {
    ...
    for (const bug of bugs) {
      ...
      lines.push(`**Affected:** ${bug.aiAnalysis.affectedMembers.join(', ')} (${bug.aiAnalysis.affectedRoles.join(', ')})`);
  ...
  private buildDescription(
    ...
    if (avgFrustration > 0.5) {
      lines.push(`> This bug causes significant user frustration. ${members.length >= 3 ? 'All test personas hit this issue.' : 'Multiple personas were affected.'}`);
      lines.push(``);
    }
