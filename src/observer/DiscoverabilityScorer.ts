import type { ActionLog, DiscoverabilityReport, FamilyConfig, FamilyState, FeatureDiscovery } from "../types.js";
...
export class DiscoverabilityScorer {
	score(logs: ActionLog[], families: FamilyConfig[], _familyStates: FamilyState[]): DiscoverabilityReport {
		...
		const discoveryRate =
			allFeatures.length > 0 ? featureBreakdown.filter((f) => f.discovered).length / allFeatures.length : 0;
		const organicBonus =
			featureBreakdown.filter((f) => f.discovered && f.organic).length /
			Math.max(1, featureBreakdown.filter((f) => f.discovered).length);
		const emptyCreateRate =
			emptyStateReactions.length > 0
				? emptyStateReactions.filter((e) => e.reaction === "created").length / emptyStateReactions.length
				: 0.5; // neutral if no empty states

		const overallScore = Math.round(
			discoveryRate * 40 + organicBonus * 20 + briefingCorrelation * 20 + emptyCreateRate * 20,
		);

		return { overallScore, featureBreakdown, briefingCorrelation, emptyStateReactions };
	}

	private scoreFeatureDiscovery(logs: ActionLog[], families: FamilyConfig[]): FeatureDiscovery[] {
		...
		const discovery = new Map<
			string,
			{ discoveredBy: Set<string>; firstStep: Map<string, number>; organic: boolean }
		>();
	...
	private scoreEmptyStateHandling(
		...
		for (const [, sessionLogs] of sessions) {
			...
			for (let i = 0; i < sorted.length; i++) {
				...
				const created = nextActions.some(
					(a) =>
						(a.action.startsWith("create-") || a.action.startsWith("add-") || a.action.startsWith("manage-")) &&
						a.result.success,
				);
