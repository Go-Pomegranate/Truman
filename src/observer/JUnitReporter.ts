...
import type { SimulationReport, ScenarioResult } from '../types.js';
...
export function writeJUnitReport(report: SimulationReport, outputPath: string): void {
  ...
  for (const [actor, results] of familyGroups) {
    ...
    lines.push(`  <testsuite name="${escapeXml(actor)}" tests="${results.length}" failures="${suiteFailures}" time="${suiteTime.toFixed(2)}">`);

    for (const result of results) {
      const testTime = (result.duration / 1000).toFixed(2);

      if (result.success) {
        lines.push(`    <testcase name="${escapeXml(result.scenarioId)}" classname="${escapeXml(actor)}" time="${testTime}" />`);
      } else {
        lines.push(`    <testcase name="${escapeXml(result.scenarioId)}" classname="${escapeXml(actor)}" time="${testTime}">`);
        ...
        lines.push(`      <failure message="${escapeXml(result.scenarioId)} failed">${escapeXml(failedCriteria)}</failure>`);
        lines.push('    </testcase>');
      }
    }
